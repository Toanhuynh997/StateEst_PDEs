function [U,V,P,q, qq, egy, egy_theta, perS, Xp, Yp] = ...
...
egy = zeros(Nt+1,1); egy_theta = egy;
egy(1) = 0.5*hx*hy*(inner(U,U)+inner(V,V));
egy_theta(1) = egy(1)+theta*(q^2-1);
...
for k = 1:Nt
    ...
    a = 0.5*(inner(U2,U2)+inner(V2,V2)) ...
        +dt*mu*(inner(U2dx,U2dx)+inner(U2dy,U2dy)+inner(V2dx,V2dx)+inner(V2dy,V2dy));
    b = inner(U-U1,U2)+inner(V-V1,V2)-dt*(inner(UgradU1,U1)+inner(UgradU2,V1));
    c = -0.5*(inner(U-U1,U-U1)+inner(V-V1,V-V1));
    a = a*hx*hy+theta; b = b*hx*hy; c = c*hx*hy-theta*q^2; 
    ...
    egy_theta(k+1) = egy(k+1)+theta*(q^2-1);
    qq(k+1) = q;
end
P = P1+q*P2;
toc

save bE_Taylor_Green.mat egy egy_theta qq ...
...
fprintf(1,'\n --- Running Results of BDF1 (option %d) ---\n',opt);
fprintf(1,'\n k = %g\n',k)
...
% hold on
% plot(TT,egy_theta,LineWidth=2)
% hold on
% plot(TT,egy_ex,LineWidth=2)
% legend('\boldmath$\mathcal{K}(u^n)$','\boldmath$\mathcal{K}_{\theta}(u^n)$',...
