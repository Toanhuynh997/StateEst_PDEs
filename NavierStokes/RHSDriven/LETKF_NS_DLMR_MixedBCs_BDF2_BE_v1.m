...
%% Running LETKF
opt = 1;
grid_U = [Xu(:), Yu(:)];    % [num_U_pts x 2]
grid_V = [Xv(:), Yv(:)];    % [num_V_pts x 2]
grid_P = [Xp(:), Yp(:)];    % [num_P_pts x 2]

% Vertically stack all grid coordinates
grid_total = [grid_U; grid_V; grid_P];  % [m_g x 2]

% Select observation coordinates
obs_xy = grid_total(spa_indices(1:end-1), :); 
...
for i = 1:filtering_steps
    ...
    ff1 = ff1+0.0001*randn(size(Xu, 1), size(Xu, 2), ensemble_size);
    ff2 = ff2+0.0001*randn(size(Xv, 1), size(Xv, 2), ensemble_size);
    ...
    R = obs_sigma^2*eye(size(spa_indices, 2));
...
%     rho = 2
    rho = 4;
    radius = 2;
    Xa_LETKF = LETKF_local_matlab(Xb, y, R, Nx, Ny, spa_indices, ...
        grid_total, obs_xy, radius, rho);
    
    x_scale = Xa_LETKF';

    x0_EnSF = x_scale(:, spa_indices);
...
save('LETKF_NS_MixedBCs_opt1_Nt2000_BDF2Ref_BE_DoubleEnSF_20Obs_v1.mat',...
    'est_save','rmse_all')